...
class GMM:
    def __init__(self, data, k, mean_selection='data_points'):
        ...
        self.covariances = np.array([np.eye(data.shape[1])] * k)
    ...
    def get_likelihood_each_sample(self, data):
        ...
        for k in range(self.k):
            mul_normal = multivariate_normal.pdf(data, self.means[k], self.covariances[k])
            likelihood += self.weights[k] * mul_normal
        return likelihood
    
    def fit(self, max_iter=100, verbose=False, plot=False):
        if self.m > 2 and plot:
...
class GMM_Runner:
    ...
    def run_gmm(self):
        for i, k in enumerate(self.k_range):
            ...
            self.likelihoods[i] = gmm.fit(max_iter=500, verbose=False)
        
        # plot_2D_data(self.k_range, self.likelihoods, "k vs Log-Likelihood Plot", "k", "Log-Likelihood")
        # max from likelihoods
        max_likelihood = np.max(self.likelihoods)
        max_k = self.k_range[np.argmax(self.likelihoods)]
        print("Max likelihood: ", max_likelihood, " for k = ", max_k)
        
        return max_k # return k with max likelihood(k*)
    
    def run_gmm_for_max_k(self, max_k):
        gmm = GMM(self.data, max_k)
        gmm.fit(max_iter=200, verbose=False, plot=True)
...
if __name__ == "__main__":
    data = load_data('data2D.txt')
    print(data.shape)
    gmm_runner = GMM_Runner(data, range(1,11))
    k_star = gmm_runner.run_gmm()
    gmm_runner.run_gmm_for_max_k(3)